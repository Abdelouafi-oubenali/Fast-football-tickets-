...
<div class="p-8 bg-gradient-to-br from-white to-green-50 rounded-xl shadow-lg ml-64 max-w-6xl">
    ...
    <form action="{{ route('match.store') }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    <select id="stade" name="Stadium" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                        ...
                        @foreach($stades as $stade)
                            <option value="{{ $stade->name }}">{{ $stade->name }} ({{ $stade->ville }})</option>
                        @endforeach
                    </select>
                    @error('stade_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="time" id="heure" name="time" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    @error('heure')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex justify-end space-x-3 mt-8">
            <a href="\match" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
